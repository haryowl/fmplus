...
- Board **Reminders** inbox lists platform alerts (`due_soon` / `overdue` / `next_due`); Ack clears them. **Check reminders** runs evaluation now (also every ~15 min server-side). `next_due` fires **once** when a scheduled follow-up job is first created after **manager** Done (field `/m` Done does **not** auto-create a next job — that caused lookalike Due rows and hid completed sheets). Unassigned follow-ups stay off the open board until assigned.
