...
- Board **Reminders** inbox lists platform alerts (`due_soon` / `overdue` / `next_due`); Ack clears them. **Check reminders** runs evaluation now (also every ~15 min server-side). `next_due` fires **once** when a scheduled follow-up job is first created after Done (not on Start/Save, and not again if Done is retried).
