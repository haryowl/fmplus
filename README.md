...
- Board **Reminders** inbox lists platform alerts (`due_soon` / `overdue` / `next_due`); Ack clears them. **Check reminders** runs evaluation now (also every ~15 min server-side). After **Done**, if the job has a schedule, one **next-cycle** job is created with due date/meters rolled **forward from completion** (not the same due window). It stays **unassigned** (hidden from Open until assigned). `next_due` notifies once when that follow-up is created; `due_soon` / `overdue` fire later from lead-time rules.
